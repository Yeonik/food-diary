...
@section('title', $recipe ? 'Edit recipe' : 'Define a recipe')

@section('content')
    <h1>{{ $recipe ? 'Edit recipe' : 'Define a recipe' }}</h1>
    <p class="muted">
        A recipe is a list of library items with weights. Its profile is computed
        from them, so a home-cooked dish behaves like any other food once defined.
        An ingredient may itself be a recipe; cycles are rejected.
    </p>
    ...
    <form method="post" action="{{ $action }}" class="panel">
        ...
        <table id="ingredients">
            <thead><tr><th>Item</th><th>Grams</th><th></th></tr></thead>
            <tbody>
                @foreach ($existing as $i => $line)
                    <tr>
                        ...
                        <td><button type="button" class="link" onclick="this.closest('tr').remove()">Remove</button></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        <p><button type="button" class="link" onclick="addIngredientRow()">+ Add ingredient</button></p>

        <p style="margin-top:12px"><button type="submit">Save recipe</button>
